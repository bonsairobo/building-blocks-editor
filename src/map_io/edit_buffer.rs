use crate::{
    map::{empty_sdf_chunk_hash_map, CompressibleSdfChunkMapReader, SdfArray, SdfChunkHashMap},
    CompressibleSdfChunkMap, SdfVoxelMap, VoxelType, EMPTY_SDF_VOXEL,
};
...
pub struct EditBuffer {
    ...
    dirty_chunk_keys: SmallKeyHashSet<Point3i>,
}
...
impl EditBuffer {
    ...
    pub fn edit_voxels_out_of_place(
        ...
        // modified yet.
        for chunk_key in reader.indexer.chunk_keys_for_extent(&extent) {
            self.edited_voxels
    ...
    pub fn insert_chunk(&mut self, touch_neighbors: bool, chunk_key: Point3i, chunk: SdfArray) {
        ...
        self.dirty_chunks_for_extent(touch_neighbors, extent);
        self.edited_voxels.write_chunk(chunk_key, chunk);
    }
    ...
    pub fn merge_edits(self, dst_map: &mut CompressibleSdfChunkMap) -> DirtyChunks {
        ...
        let edited_chunk_keys = chunk_storage.chunk_keys().cloned().collect();
    ...
    fn dirty_chunks_for_extent(&mut self, touch_neighbors: bool, extent: Extent3i) {
        ...
        for chunk_key in self
            .edited_voxels
            .indexer
            .chunk_keys_for_extent(&dirty_extent)
        {
...
pub struct DirtyChunks {
    ...
    pub dirty_chunk_keys: SmallKeyHashSet<Point3i>,
}

/// Merges edits from the `EditBuffer` into the `SdfVoxelMap`. By setting the `DirtyChunks` resource, the `chunk_processor_system`
/// will be notified to process dirty chunks on the next frame.
